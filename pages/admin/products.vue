                                    ...
                                    <div class="mb-4">
                                        <div
                                            ...
                                            <draggable v-model="productImages" v-if="productImages.length > 0"
                                                class="grid grid-cols-2 gap-4" item-key="id" @change="updateOrder">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <div class="relative">
                                                ...
                                                <input type="text" id="price" v-model="formattedPrice"
                                                    class="w-full pl-10 pr-3 py-2 dark:text-white border border-gray-300 rounded-md focus:outline-none focus:ring-orange-500 focus:border-orange-500"
        ...
        if (file.type.startsWith('image/')) {
            ...
            reader.onload = (e) => {
                productImages.value.push({
                    ...
                    order: productImages.value.length + 1 // Tambahkan order saat menambah gambar
                })
            }
...
function openModal(selectedCategory = null) {
    if (selectedCategory) {
        productForm.value.id = selectedCategory.id
        productForm.value.name = selectedCategory.name
        productForm.value.description = selectedCategory.description
...
const createNewProduct = async () => {
    try {
        ...
        // If products is a base64 string, convert it to a file first
        ...
        if (isCreateNewCategory.value) {
            ...
            await $fetch(`/categories/${productForm.value.id}`, {
                method: 'PUT',
                body: formData,
                baseURL: apiUrl,
                credentials: 'include'
